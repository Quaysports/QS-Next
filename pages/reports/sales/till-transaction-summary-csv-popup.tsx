...
export default function TillTransactionSummaryCSVPopup() {
  ...
  function handler(event: FormEvent<HTMLFormElement>) {
    ...
    fetch("/api/reports/get-till-transaction-csv-data", opts)
      .then((res) => res.json())
      .then((json) => {
        let map: Map<string, { total: number; card: number; cash: number }> =
          new Map();

        const vatCalc = (total: number) =>
          ((total / 100 / 120) * 20).toFixed(2);
        ...
        for (let [k, v] of map)
          csvData += `\r\n${k}, ${toCurrencyInput(v.total)}, ${vatCalc(
            v.total
          )}, ${toCurrencyInput(v.card)}, ${vatCalc(v.card)}, ${toCurrencyInput(
            v.cash
          )}, ${vatCalc(v.cash)}`;
        ...
        link.setAttribute(
          "download",
          `till-transactions-${formData.get("from")}-${formData.get("to")}.csv`
        );
