...
function TitleRow() {
  return (
    <div className={styles["info-row"]}>
      <div
        style={{ opacity: 0 }}
        className={calendarStyles["booked-dot"]}
      ></div>
      <div>Name</div>
...
function UserRow({ user }: { user: User }) {
  const { bookedDays, paidSickDays, unpaidSickDays } =
    useSelector(selectBookedTotals);
  ...
  const holiday = user.holiday?.find(
    (userHoliday) => userHoliday.year === cy
  ) || { year: cy, days: 0 };
  ...
  return (
    <div className={styles["info-row"]}>
      <div
        ...
        className={calendarStyles["booked-dot-100"]}
      ></div>
