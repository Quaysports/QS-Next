...
export async function middleware(request:NextRequest) {
    ...
    switch(request.nextUrl.pathname) {
        case '/dashboard?tab=users':        return user.permissions.users?.auth ?              null: rootRedirect();
        case '/dashboard?tab=orders':       return user.permissions.orderSearch?.auth ?        null: rootRedirect();
        case '/dashboard?tab=price':        return user.permissions.priceUpdates?.auth ?       null: rootRedirect();
        case '/dashboard?tab=shop':         return user.permissions.shop?.auth ?               null: rootRedirect();
        case '/dashboard?tab=bait':         return user.permissions.baitOrdering?.auth ?       null: rootRedirect();
        case '/dashboard?tab=online':       return user.permissions.online?.auth ?             null: rootRedirect();
        case '/dashboard?tab=rotas':        return user.permissions.rotas?.auth ?              null: rootRedirect();
        case '/dashboard?tab=holidays':     return user.permissions.holidays?.auth ?           null: rootRedirect();
        ...
        case '/shop-tills?tab=quick-links': return user.permissions.shopTillsQuickLinks?.auth ? null: rootRedirect();
